	...
	extends RetrievalQuery<TSelectedCols, TSingleColumn>
{
	 ...
	 */
	public forUpdate(this: SelectQuery<TSelectedCols, TFromTblCols, TSingleColumn, false>): SelectQuery<TSelectedCols, TFromTblCols, TSingleColumn, true>;
	public forUpdate(this: ErrorMessage<"forUpdate() can only be called once per query.">): "Error";
	public forUpdate(this: any): any {
...
export const select = secondWithTypeOfFirst(new SelectQuery<{}, {}, NoColumnsSelected, false>().select, function (...args: any[]): any {
		const result = new SelectQuery<{}, {}, NoColumnsSelected, false>();
		return (result.select as any).call(result, ...args);
});
